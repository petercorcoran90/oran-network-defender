...
| 2026-06-01 | Team + Claude | Step 3: persistence/integration tests + retire H2 _(pending review)_ | Removed H2 dep + smoke test + `data.sql`; added Testcontainers (MySQL 8); `AbstractMySqlIntegrationTest` (one shared container), `Fixtures`, `PersistenceIntegrationTest` (mappings/enums/finders), `SubmitActionIntegrationTest` (correct→heal/score, trap→fail/cascade, ineffective→stays open, cross-player rejected) | Compiles + runs; **verified on CI** (local Docker Desktop 29.x returns HTTP 400 to docker-java so they can't run on this machine); **human review pending** | Real MySQL replaces H2; tests need Docker (CI has it). Local fix: enable Docker Desktop's default `/var/run/docker.sock` |
