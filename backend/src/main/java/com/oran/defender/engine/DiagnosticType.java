...
public enum DiagnosticType {
    TRACE_TRANSPORT("Trace transport link", RootCause.TRANSPORT_LINK_FAULT),
    CHECK_NEIGHBOUR_CONFIG("Check neighbour configuration", RootCause.NEIGHBOUR_CONFIG_CHANGE),
    CHECK_UPGRADE_HISTORY("Check software-upgrade history", RootCause.SOFTWARE_UPGRADE_FAULT),
    RADIO_SCAN("Scan the radio environment", RootCause.NEIGHBOUR_INTERFERENCE),
    INSPECT_AUTOMATION("Inspect automation logs", RootCause.ROGUE_AUTOMATION),
    CORRELATE_ALARMS("Correlate the alarm burst", RootCause.ALARM_STORM);

    private final String label;
    private final RootCause implicates;

    DiagnosticType(String label, RootCause implicates) {
        this.label = label;
